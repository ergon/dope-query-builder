...
interface SelectClauseResolver : MergeableClauseResolver {
    fun resolve(selectClause: ISelectOffsetClause<*>): CouchbaseDopeQuery =
        when (selectClause) {
            is SelectClause -> {
                ...
                CouchbaseDopeQuery(
                    queryString =
                        formatQueryStringWithNullableFirst(
                            parentDopeQuery,
                            "SELECT",
                            expressionDopeQuery,
                    ...
                    parameters =
                        (parentDopeQuery?.parameters.orEmpty()).merge(
            ...
            is SelectDistinctClause -> {
                ...
                CouchbaseDopeQuery(
                    ...
                    parameters =
                        expressionDopeQuery.parameters.merge(
            ...
            is FromClause<*> -> {
                val parentDopeQuery = selectClause.parentClause.toDopeQuery(this)
                val fromableDopeQuery =
                    when (val fromable = selectClause.fromable) {
            ...
            is LetClause<*> -> {
                ...
                val additionalVariableAssignments =
                    selectClause.dopeVariables.map { variable ->
            ...
            is SetOperator<*> -> {
                ...
                CouchbaseDopeQuery(
                    queryString =
                        "(${left.queryString}) ${selectClause.setOperatorType} $all (${right.queryString})".replace(
                            "  ",
                            " ",
                        ),
