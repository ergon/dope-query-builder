...
class StrToMillisExpressionTest : ResolverDependentTest {
    ...
    fun `should support STR_TO_MILLIS with positional format parameter`() {
        ...
        val expected = CouchbaseDopeQuery("STR_TO_MILLIS(`stringField`, $1)", DopeParameters(positionalParameters = listOf("yyyy")))
        assertEquals(expected, underTest.toDopeQuery(resolver))
    }
    ...
    fun `should support STR_TO_MILLIS with named format parameter`() {
        ...
        val expected = CouchbaseDopeQuery("STR_TO_MILLIS(`stringField`, \$f)", DopeParameters(namedParameters = mapOf("f" to "fmt")))
        assertEquals(expected, underTest.toDopeQuery(resolver))
    }
