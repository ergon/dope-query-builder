...
class NowTimezoneExpressionTest : ResolverDependentTest {
    ...
    fun `should support NOW_TZ with positional parameter timezone`() {
        ...
        val expected = CouchbaseDopeQuery("NOW_TZ($1)", DopeParameters(positionalParameters = listOf("UTC")))
        assertEquals(expected, expr.toDopeQuery(resolver))
    }
    ...
    fun `should support NOW_TZ with named parameter timezone`() {
        ...
        val expected = CouchbaseDopeQuery("NOW_TZ($" + "zone)", DopeParameters(namedParameters = mapOf("zone" to "UTC")))
        assertEquals(expected, expr.toDopeQuery(resolver))
    }
    ...
    fun `should support NOW_TZ with timezone and format fields`() {
        ...
        val expected = CouchbaseDopeQuery("NOW_TZ(`stringField`, `stringField`)")
        assertEquals(expected, expr.toDopeQuery(resolver))
    }
