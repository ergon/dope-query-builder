...
class RangeTransformationsTest {
    ...
    fun `should filter and transform array`() {
        val expected =
            "SELECT ARRAY (`iterator1` * `iterator1`) FOR `iterator1` IN `numberArrayField` " +
    ...
    fun `should filter and transform array with indices`() {
        val expected =
            "SELECT ARRAY (`it` * `it`) FOR `i`:`it` IN `numberArrayField` " +
                "WHEN `i` <= 2 END[0] = 9"
    ...
    fun `should filter and transform unnested array`() {
        val expected =
            "SELECT ARRAY `it` FOR `i`:`it` WITHIN `anyTypeArrayField` " +
                "WHEN `i` <= 2 END"
    ...
    fun `should transform array and get first instance to use in string function`() {
        val expected =
            "SELECT CONCAT(\"test\", FIRST `it` FOR `i`:`it` IN `stringArrayField` " +
                "WHEN `i` <= 2 END)"
