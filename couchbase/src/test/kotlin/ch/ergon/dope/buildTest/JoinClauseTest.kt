...
class JoinClauseTest {
    ...
    val airline = someBucket("airline")
        ...
        val actual =
            QueryBuilder
                ...
                .from(
                    ...
                    airline,
                    condition =
                        someStringField("airlineid", route).isEqualTo(
                            meta(airline).id,
                        ),
                ).where(
    ...
    fun `Should support complex inner join`() {
        val expected =
            "SELECT `route`.`airlineid`, `airline`.`iota`, `route`.`sourceairport`, " +
    ...
    fun `Left Outer Join of US airports in the same city as a landmark`() {
        ...
        val actual =
            QueryBuilder.selectDistinct(
                ...
                lmark,
                condition =
                    someStringField("city", aport).isEqualTo(
    ...
    fun `Inner Lookup Join`() {
        val expected =
            "SELECT DISTINCT `route`.`destinationairport`, `route`.`stops`, " +
    ...
    fun `Left Outer Lookup Join`() {
        val expected =
            "SELECT `route`.`airline`, `route`.`sourceairport`, " +
        ...
        val actual =
            QueryBuilder
                ...
                .from(
                    ...
                    airlineBucket,
                    condition =
                        someStringField("airlineid", routeBucket).isEqualTo(
