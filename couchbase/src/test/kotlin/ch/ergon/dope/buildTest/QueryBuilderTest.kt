...
class QueryBuilderTest : ResolverDependentTest {
    ...
    fun `Should support the NOW_STR function`() {
        val expected =
            "SELECT NOW_STR() AS `full_date`, NOW_STR(\"invalid date\") AS `invalid_date`, " +
                "NOW_STR(\"1111-11-11\") " +
                "AS `short_date`"
    ...
    fun `should support selecting case`() {
        val someBucket = someBucket()
        val expected =
            "SELECT CASE `numberField` WHEN `other` THEN 2 END, " +
    ...
    fun `should support selecting let clause`() {
        ...
        val expected =
            "SELECT `t1`.`destinationAirport`, `equip` AS `has_radio` " +
                "FROM `route` AS `t1` " +
    ...
    fun `should support with clause before query`() {
        ...
        val avgLikeCount =
            "avgLikeCount".assignTo(
                QueryBuilder.selectRaw(avg(ctePublicLikes)).from(cte).asExpression(),
            )

        val expected =
            "WITH `avgLikeCount` AS ((SELECT RAW AVG(`cte`.`publicLikes`) FROM `hotel` AS `cte`)) " +
