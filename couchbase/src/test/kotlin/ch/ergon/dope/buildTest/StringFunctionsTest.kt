...
class StringFunctionsTest : ResolverDependentTest {
    ...
    fun `should Support Ltrim`() {
        val expected =
            "SELECT LTRIM(\"...N1QL is awesome\", \".\") AS `dots`, LTRIM(\"    N1QL is awesome\", \" \") AS " +
    ...
    fun `should Support Tokens`() {
        val expected =
            "SELECT TOKENS([\"jim@example.com\", \"kim@example.com\", \"https://example.com/\", \"408-555-1212\"], " +
                "{\"name\": true})"
    ...
    fun `should support string functions with number types position`() {
        val expected =
            "SELECT (POSITION1(\"input\", \"i\") + POSITION(\"input\", \"n\")) " +
    ...
    fun `should support string functions with number types other`() {
        val expected =
            "SELECT CONCAT(INITCAP(\"input\"), MASK(\"input\", {}), REPEAT(\"input\", 3), " +
