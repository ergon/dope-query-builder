...
class ObjectForRangeTest : ManagerDependentTest {
    ...
    @Test
    fun `should support object range transformation with cm number list`() {
    ...
    @Test
    fun `should support object range transformation with cm string list`() {
    ...
    @Test
    fun `should support object range transformation with cm boolean list`() {
    ...
    @Test
    fun `should support object unnested range transformation with cm number list`() {
        val range = someCMNumberList()
        val iteratorName = "it"
        val indexName = "i"
        val expected = ObjectForRangeExpression(
        ...
        val actual = range.mapIndexedUnnested(iteratorName, indexName) { it, i -> it.add(i) }.toObject { _, i -> i.toStr() }

        assertEquals(expected.toDopeQuery(manager), actual.toDopeQuery(manager))
    }

    @Test
    fun `should support object unnested range transformation with cm string list`() {
        val range = someCMStringList()
        val iteratorName = "it"
        val indexName = "i"
        val expected = ObjectForRangeExpression(
        ...
        val actual = range.mapIndexedUnnested(iteratorName, indexName) { it, i -> concat(it, i.toStr()) }.toObject { _, i -> i.toStr() }

        assertEquals(expected.toDopeQuery(manager), actual.toDopeQuery(manager))
    }

    @Test
    fun `should support object unnested range transformation with cm boolean list`() {
        val range = someCMBooleanList()
        val iteratorName = "it"
        val indexName = "i"
        val expected = ObjectForRangeExpression(
        ...
        val actual = range.mapIndexedUnnested(iteratorName, indexName) { it, i -> it.and(i.toBool()) }.toObject { _, i -> i.toStr() }

        assertEquals(expected.toDopeQuery(manager), actual.toDopeQuery(manager))
    }

    @Test
    fun `should support object range transformation with condition with cm number list`() {
        val range = someCMNumberList()
        val iteratorName = "it"
        val indexName = "i"
        val expected = ObjectForRangeExpression(
        ...
        val actual = range.filter { it, i -> it.isEqualTo(i) }.mapIndexed(
            iteratorName,
            indexName,
        ) { it, i -> it.add(i) }.toObject { _, i -> i.toStr() }

        assertEquals(expected.toDopeQuery(manager), actual.toDopeQuery(manager))
    }

    @Test
    fun `should support object range transformation with condition with cm string list`() {
        val range = someCMStringList()
        val iteratorName = "it"
        val indexName = "i"
        val expected = ObjectForRangeExpression(
        ...
        val actual = range.filter { it, i -> contains(it, i.toStr()) }.mapIndexed(
            iteratorName,
            indexName,
        ) { it, i -> repeat(it, i) }.toObject { _, i -> i.toStr() }

        assertEquals(expected.toDopeQuery(manager), actual.toDopeQuery(manager))
    }

    @Test
    fun `should support object range transformation with condition with cm boolean list`() {
        val range = someCMBooleanList()
        val iteratorName = "it"
        val indexName = "i"
        val expected = ObjectForRangeExpression(
        ...
        val actual = range.filter { it, i -> it.or(i.toBool()) }.mapIndexed(
            iteratorName,
            indexName,
        ) { it, i -> it.and(i.toBool()) }.toObject { _, i -> i.toStr() }

        assertEquals(expected.toDopeQuery(manager), actual.toDopeQuery(manager))
    }

    @Test
    fun `should support object unnested range transformation with condition with cm number list`() {
        val range = someCMNumberList()
        val iteratorName = "it"
        val indexName = "i"
        val expected = ObjectForRangeExpression(
    ...
    @Test
    fun `should support object unnested range transformation with condition with cm string list`() {
        val range = someCMStringList()
        val iteratorName = "it"
        val indexName = "i"
        val expected = ObjectForRangeExpression(
    ...
    @Test
    fun `should support object unnested range transformation with condition with cm boolean list`() {
        val range = someCMBooleanList()
        val iteratorName = "it"
        val indexName = "i"
        val expected = ObjectForRangeExpression(
